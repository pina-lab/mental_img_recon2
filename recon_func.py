...
def clip_extreme(x, pct=1):
    # referred from Shen et al., 2019 public code.
    if pct < 0:
        pct = 0.
    if pct > 100:
        pct = 100.
    x = np.clip(x, np.percentile(x, pct / 2.),
...
class imageRecon():
    ...
    def withoutVQGAN(self, initInput=None, initInputType='PIL', optimizer='Adam', lr=0.5, T=1.0, numReps=None, returnVec=False):
        ...
        # Start iterative optimization.
        for index_rep in tqdm(range(numReps), ncols=100, desc="Progress rate"):

            # The latent vector is converted into the corresponding image.
            VQGANoutput_VGG = convertVQGANoutputIntoVGGinput(
                currentLatentVector)
            VQGANoutput_CLIP = convertVQGANoutputIntoCLIPinput(
                currentLatentVector)
            ...
            if len(self.usedLayer) == 0:
                ...
                loss_VGG = compute_loss_VGG(self.VGGmodel, self.usedLayer, self.layerWeight, VQGANoutput_VGG,
                                            'img', self.targetVGGfeature, 'feat', self.meanVGGfeature, cosSimilarity)
            ...
            # Plot the results.
            if (index_rep+1) % self.disp_every == 0:
                output_im = currentLatentVector.detach().clone().to(self.DEVICE)
                output_im = output_im.to('cpu').detach().numpy().copy()
                recImg = normalize_image(output_im)
                del output_im
                yield recImg, index_rep+1, loss_VGG_np, loss_CLIP_np, None
